...
現在は **Phase 11.3 完了 / v1.0.2 安定化版** です。
...
6. `build/libs/questadmin-1.0.2.jar` が生成されることを確認します。
...
3. サーバーの `mods` フォルダへ `questadmin-1.0.2.jar` を入れます。
...
- Phase 11.4 GUI軽量化 / ページング / 所持数計算キャッシュ
- JSONバックアップ/リストア補助
- 新しいクエスト種別の設計検討
- 村人/NPC連携の設計検討

Phase 11.3時点では、新機能追加ではなく v1.0.2 安定化対応までを完了しています。
