...
現在は **Phase 11.6 完了 / v1.0.5 安定化版** です。
...
6. `build/libs/questadmin-1.0.5.jar` が生成されることを確認します。
...
3. サーバーの `mods` フォルダへ `questadmin-1.0.5.jar` を入れます。
...
8. `/questadmin` で管理者GUIが開くことを確認します。
9. `/quest` でプレイヤーGUIが開くことを確認します。
10. クエストを新規作成できることを確認します。
11. 作成したクエストを編集できることを確認します。
12. enabled 切替が反映されることを確認します。
13. 削除確認付きで削除できることを確認します。
14. `/quest list` に有効クエストのみがページ表示されることを確認します。
15. `/quest complete <questId>` で完了できることを確認します。
16. `/quest claim <questId>` で報酬が入金されることを確認します。
17. 二重claimできないことを確認します。
18. 保存時間ログが過剰に出ていないことを確認します。
19. 再起動後も `quests.json` と `player_quests.json` が維持されることを確認します。
20. `latest.log` に questadmin 由来の ERROR が出ていないことを確認します。
...
- Phase 11.7 保存リトライ/バックアップ設計
- Phase 12 新機能検討
- JSONバックアップ/リストア補助
- 新しいクエスト種別の設計検討
- 村人/NPC連携の設計検討

Phase 11.6時点では、新機能追加ではなく v1.0.5 安定化対応までを完了しています。
