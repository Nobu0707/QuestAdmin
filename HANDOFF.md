...
現在は **Phase 11.7 完了 / v1.0.6 安定化版** です。
...
- `/questadmin storage status` による保存I/O計測状態の確認
- `/questadmin storage backup` による手動バックアップ作成
- `/questadmin storage backups` によるバックアップ一覧表示
- `/questadmin storage validate` による保存ファイル検証
- 保存に50ms以上かかった場合のwarnログ、200ms以上の強いwarnログ
- enabled の有効/無効切替
- 削除確認GUI経由のクエスト削除
- チャット入力ステップ方式のクエスト作成
- チャット入力ステップ方式のクエスト編集
...
`/questadmin storage backup` で、以下へタイムスタンプ付きバックアップを作成できます。

```text
config/questadmin/backups/
```

バックアップは `quests.json` と `player_quests.json` それぞれ最新10件程度に制限されます。
QuestAdminは自動復元や復元コマンドを提供しません。復元が必要な場合はサーバーを停止し、現在のJSONを別場所へ退避したうえで、バックアップファイルを手動で元のファイル名へコピーしてください。
...
6. `build/libs/questadmin-1.0.6.jar` が生成されることを確認します。
...
3. サーバーの `mods` フォルダへ `questadmin-1.0.6.jar` を入れます。
...
8. `/questadmin storage backup` でバックアップが作成されることを確認します。
9. `/questadmin storage backups` でバックアップ一覧が表示されることを確認します。
10. `/questadmin storage validate` で正常ファイルがOK表示されることを確認します。
11. `/questadmin` で管理者GUIが開くことを確認します。
12. `/quest` でプレイヤーGUIが開くことを確認します。
13. クエストを新規作成できることを確認します。
14. 作成したクエストを編集できることを確認します。
15. enabled 切替が反映されることを確認します。
16. 削除確認付きで削除できることを確認します。
17. `/quest list` に有効クエストのみがページ表示されることを確認します。
18. `/quest complete <questId>` で完了できることを確認します。
19. `/quest claim <questId>` で報酬が入金されることを確認します。
20. 二重claimできないことを確認します。
21. 保存時間ログが過剰に出ていないことを確認します。
22. 再起動後も `quests.json` と `player_quests.json` が維持されることを確認します。
23. `latest.log` に questadmin 由来の ERROR が出ていないことを確認します。
...
- `quests.json` / `player_quests.json` の保存時間を計測します。
- 保存成功回数、保存失敗回数、atomic fallback回数を軽量なstatic metricsで保持します。
- `/questadmin storage status` で保存状態を確認できます。
- 保存に50ms以上かかった場合はwarnログ、200ms以上ではより強いwarnログを出します。
- 安全性優先のため、`/quest complete <questId>`、`/quest claim <questId>`、`/questadmin progress mark ...` は同期保存のままです。
- 保存方針と将来の非同期/debounce設計メモは `docs/STORAGE_IO_STRATEGY.md` にまとめています。

## 20. Phase 11.7 メモ

- `/questadmin storage backup` で `quests.json` / `player_quests.json` を `config/questadmin/backups/` へ手動バックアップします。
- `/questadmin storage backups` で直近のバックアップを最大10件表示します。
- `/questadmin storage validate` で現在の保存ファイルを読み込み専用で検証します。
- バックアップは `quests.json` / `player_quests.json` それぞれ最新10件程度に制限します。
- 保存のたびの自動バックアップ、自動復元、復元コマンドは実装していません。
- `/quest complete <questId>`、`/quest claim <questId>`、`/questadmin progress mark ...` は同期保存のままです。

## 21. 次期Phase候補

- Phase 11.8 運用ドキュメント最終化
- Phase 12 新機能検討
- 新しいクエスト種別の設計検討
- 村人/NPC連携の設計検討

Phase 11.7時点では、新機能追加ではなく v1.0.6 安定化対応までを完了しています。
